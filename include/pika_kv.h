...
class SetOnceCmd : public Cmd {
public:
    enum SetCondition {kNONE, kNX, kXX, kVX, kEXORPX};
    SetOnceCmd(const std::string& name , int arity, uint16_t flag)
            : Cmd(name, arity, flag), sec_(0), condition_(kNONE) {};
    virtual std::vector<std::string> current_key() const {
        std::vector<std::string> res;
        res.push_back(key_);
...
private:
    ...
    int32_t success_;
    int64_t sec_;
    SetOnceCmd::SetCondition condition_;
    virtual void DoInitial() override;
    virtual void Clear() override {
        sec_ = 0;
        success_ = 0;
...
class IncrbyrangeCmd : public Cmd {
 public:
  IncrbyrangeCmd(const std::string& name , int arity, uint16_t flag)
      : Cmd(name, arity, flag) {};
  virtual std::vector<std::string> current_key() const {
    std::vector<std::string> res;
    res.push_back(key_);
  ...
  virtual void Do(std::shared_ptr<Partition> partition = nullptr);
  virtual Cmd* Clone() override {
    return new IncrbyrangeCmd(*this);
  }
 private:
  std::string key_;
  int64_t by_, new_value_,min_,max_;
  virtual void DoInitial() override;
};
