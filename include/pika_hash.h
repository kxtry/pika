...
class HSetOnceCmd : public Cmd {
public:
    HSetOnceCmd(const std::string& name, int arity, uint16_t flag)
            : Cmd(name,  arity, flag) {}
    virtual std::vector<std::string> current_key() const {
        std::vector<std::string> res;
        res.push_back(key_);
...
class HIncrbyrangeCmd : public Cmd {
 public:
  HIncrbyrangeCmd(const std::string& name, int arity, uint16_t flag)
      : Cmd(name,  arity, flag) {}
  virtual std::vector<std::string> current_key() const {
    std::vector<std::string> res;
    res.push_back(key_);
  ...
  virtual void Do(std::shared_ptr<Partition> partition = nullptr);
  virtual Cmd* Clone() override {
    return new HIncrbyrangeCmd(*this);
  }
 private:
  std::string key_, field_;
  int64_t by_,min_,max_;
  virtual void DoInitial() override;
};
