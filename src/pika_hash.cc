...
void HSetOnceCmd::DoInitial() {
    if (!CheckArg(argv_.size())) {
        res_.SetRes(CmdRes::kWrongNum, kCmdNameHSetOnce);
        return;
    }
    key_ = argv_[1];
    field_ = argv_[2];
...
void HSetOnceCmd::Do(std::shared_ptr<Partition> partition) {
    int32_t ret = 0;
    rocksdb::Status s = partition->db()->HSetOnce(key_, field_, value_, &ret);
    if (s.ok()) {
        res_.AppendContent(":" + std::to_string(ret));
    } else {
        res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void HIncrbyrangeCmd::DoInitial() {
  if (!CheckArg(argv_.size())) {
    res_.SetRes(CmdRes::kWrongNum, kCmdNameHIncrbyrange);
    return;
  }
  key_ = argv_[1];
  field_ = argv_[2];
  if (!slash::string2l(argv_[3].data(), argv_[3].size(), &by_)) {
    res_.SetRes(CmdRes::kInvalidInt, kCmdNameHIncrbyrange);
    return;
  }
  if (!slash::string2l(argv_[4].data(), argv_[4].size(), &min_)) {
    res_.SetRes(CmdRes::kInvalidInt, kCmdNameHIncrbyrange);
    return;
  }
  if (!slash::string2l(argv_[5].data(), argv_[5].size(), &max_)) {
    res_.SetRes(CmdRes::kInvalidInt, kCmdNameHIncrbyrange);
    return;
  }
  return;
...
void HIncrbyrangeCmd::Do(std::shared_ptr<Partition> partition) {
  int64_t new_value;
  rocksdb::Status s = partition->db()->HIncrbyrange(key_, field_, by_, &new_value, min_, max_);
  if (s.ok() || s.IsNotFound()) {
    res_.AppendContent(":" + std::to_string(new_value));
  } else if (s.IsCorruption() && s.ToString() == "Corruption: hash value is not an integer") {
    res_.SetRes(CmdRes::kInvalidInt);
