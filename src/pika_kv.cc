...
/* SETONCE key value*/
void SetOnceCmd::DoInitial() {
    if (!CheckArg(argv_.size())) {
        res_.SetRes(CmdRes::kWrongNum, kCmdNameSetOnce);
        return;
    }
    key_ = argv_[1];
    value_ = argv_[2];
    condition_ = SetOnceCmd::kNONE;
    sec_ = 0;
    return;
}

void SetOnceCmd::Do(std::shared_ptr<Partition> partition) {
    rocksdb::Status s;
    int32_t res = 1;
    s = partition->db()->SetOnce(key_, value_);
...
std::string SetOnceCmd::ToBinlog(
        uint32_t exec_time,
        const std::string& server_id,
        uint64_t logic_id,
        uint32_t filenum,
...
void IncrbyrangeCmd::DoInitial() {
  if (!CheckArg(argv_.size())) {
    res_.SetRes(CmdRes::kWrongNum, kCmdNameIncrbyrange);
    return;
  }
  key_ = argv_[1];
  if (!slash::string2l(argv_[2].data(), argv_[2].size(), &by_)) {
    res_.SetRes(CmdRes::kInvalidInt, kCmdNameIncrbyrange);
    return;
  }
  if (!slash::string2l(argv_[3].data(), argv_[3].size(), &min_)) {
      res_.SetRes(CmdRes::kInvalidInt, kCmdNameIncrbyrange);
      return;
  }
  if (!slash::string2l(argv_[4].data(), argv_[4].size(), &max_)) {
    res_.SetRes(CmdRes::kInvalidInt, kCmdNameIncrbyrange);
    return;
  }
  return;
...
void IncrbyrangeCmd::Do(std::shared_ptr<Partition> partition) {
  rocksdb::Status s = partition->db()->Incrbyrange(key_, by_, &new_value_, min_, max_);
  if (s.ok()) {
    res_.AppendContent(":" + std::to_string(new_value_));
  } else if (s.IsCorruption() && s.ToString() == "Corruption: Value is not a integer") {
    res_.SetRes(CmdRes::kInvalidInt);
