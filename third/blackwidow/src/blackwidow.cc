...
Status BlackWidow::SetOnce(const Slice& key,
                       const Slice& value) {
    return strings_db_->SetOnce(key, value);
}
...
Status BlackWidow::Incrbyrange(const Slice& key, int64_t value, int64_t* ret, int64_t min_, int64_t max_) {
  return strings_db_->Incrbyrange(key, value, ret, min_, max_);
}
...
Status BlackWidow::HSetOnce(const Slice& key, const Slice& field,
                        const Slice& value, int32_t* res) {
    return hashes_db_->HSetOnce(key, field, value, res);
}
...
Status BlackWidow::HIncrbyrange(const Slice& key, const Slice& field, int64_t value,
                           int64_t* ret, int64_t min_, int64_t max_) {
  return hashes_db_->HIncrbyrange(key, field, value, ret, min_, max_);
}
