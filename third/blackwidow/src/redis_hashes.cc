...
Status RedisHashes::HIncrbyrange(const Slice& key, const Slice& field, int64_t value,
...
Status RedisHashes::HSetOnce(const Slice& key, const Slice& field,
                         const Slice& value, int32_t* res) {
    rocksdb::WriteBatch batch;
    ScopeRecordLock l(lock_mgr_, key);
    ...
    if (s.ok()) {
        ...
        if (parsed_hashes_meta_value.IsStale()
            ...
            if (s.ok()) {
                *res = 999;
                return Status::Aborted("give up for had been init");
            } else if (s.IsNotFound()) {
                parsed_hashes_meta_value.ModifyCount(1);
                batch.Put(handles_[0], key, meta_value);
                batch.Put(handles_[1], hashes_data_key.Encode(), value);
