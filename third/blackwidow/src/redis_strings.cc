...
Status RedisStrings::Incrbyrange(const Slice& key, int64_t value, int64_t* ret, int64_t min_, int64_t max_) {
  std::string old_value;
  std::string new_value;
  ScopeRecordLock l(lock_mgr_, key);
  Status s = db_->Get(default_read_options_, key, &old_value);
  if (s.ok()) {
    ...
    if (parsed_strings_value.IsStale()) {
      *ret = value+min_;
      ...
      if (ival < min_ || ival >= max_) {
        std::string out = "NotInRange:";
        out += old_user_value;
        return Status::InvalidArgument(out);
      }
      *ret = ival + value;
      new_value = std::to_string(*ret);
      StringsValue strings_value(new_value);
...
Status RedisStrings::SetOnce(const Slice& key,
                         const Slice& value) {
    StringsValue strings_value(value);
    ScopeRecordLock l(lock_mgr_, key);
    std::string hit;
    Status s = db_->Get(default_read_options_, key, &hit);
    if (s.ok()) {
        //return Status::Aborted("the value had been init.");
        ParsedStringsValue parsed_strings_value(&hit);
        if (!parsed_strings_value.IsStale()) {
            return Status::Aborted("the value had been init.");
        }
    }
